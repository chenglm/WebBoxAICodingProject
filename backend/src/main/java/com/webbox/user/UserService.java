...
public class UserService { private final JdbcTemplate jdbc; public UserService(JdbcTemplate jdbc){this.jdbc=jdbc;}
 public Preferences preferences(long userId){ var r=jdbc.query("select preferred_categories,spice_preference,taste_preference,budget_cents,recommended_enabled from user_preferences where user_id=?",(rs,n)->new Object[]{rs.getString(1),rs.getString(2),rs.getString(3),rs.getObject(4,Long.class),rs.getBoolean(5)},userId); if(r.isEmpty())return new Preferences(List.of(),null,null,null,List.of(),false); Object[] x=r.get(0); return new Preferences(csv((String)x[0]),(String)x[1],(String)x[2],(Long)x[3],jdbc.query("select allergen from user_allergens where user_id=?",(rs,n)->rs.getString(1),userId),(Boolean)x[4]); }
 @Transactional public Preferences savePreferences(long userId,Preferences p){ if(p.budgetCents()!=null&&p.budgetCents()<0)throw new ApiException(HttpStatus.BAD_REQUEST,"VALIDATION_ERROR","Budget cannot be negative."); jdbc.update("update user_preferences set preferred_categories=?,spice_preference=?,taste_preference=?,budget_cents=?,recommended_enabled=? where user_id=?",String.join(",",safe(p.preferredCategories())),p.spicePreference(),p.tastePreference(),p.budgetCents(),p.recommendedEnabled(),userId); jdbc.update("delete from user_allergens where user_id=?",userId); for(String a:safe(p.allergens()))jdbc.update("insert into user_allergens(user_id,allergen) values(?,?)",userId,a); return preferences(userId); }

...
public final class MenuModels {
 ...
 public record DishInput(@NotBlank @Size(max=120) String name,@NotBlank @Size(max=1000) String description,@NotBlank @Pattern(regexp="Chinese|Japanese|Vegetarian|Western|Korean",message="Unsupported category.") String category,@Size(max=60) String protein,@Pattern(regexp="None|Mild|Medium|Hot",message="Unsupported spice level.") String spiceLevel,@NotNull @PositiveOrZero Long priceCents,@Size(max=500) String imageUrl,Boolean visible,List<@Pattern(regexp="Peanuts|Soy|Fish|Gluten|Dairy|Egg",message="Unsupported allergen.") String> allergens,List<@Valid OptionGroupInput> optionGroups) {}

...
public class AdminController { private final MenuService service; public AdminController(MenuService service){this.service=service;}
 @GetMapping("/dishes") public List<Dish> dishes(@RequestParam(required=false) String q,@RequestParam(required=false) String category){return service.adminList(q,category);}
 @PostMapping("/dishes") @ResponseStatus(HttpStatus.CREATED) public Dish create(@RequestBody DishInput i){return service.create(i);}
 @PutMapping("/dishes/{id}") public Dish update(@PathVariable long id,@RequestBody DishInput i){return service.update(id,i);}
 ...
 @PostMapping("/daily-menus") @ResponseStatus(HttpStatus.NO_CONTENT) public void daily(@RequestBody DailyMenuInput input){service.configureDaily(input);}
}

...
> 每日菜单仍会维护 `availableQuantity`，为后续 Tier 3 库存能力预留数据字段；Tier 1/2 中它仅作为管理员维护的供应配置，不展示实时余量，也不参与扣减。
...
7. **取消订单**：仅 `Pending` 可取消；状态更新必须受服务端条件限制，禁止由客户端直接传入任意状态。
...
| UI | Tailwind CSS + Headless 组件 | 快速构建一致的桌面/移动端英文界面，避免重量级主题定制 |
...
菜单读路径采用 Caffeine 进程内缓存：缓存「日期 + 筛选前的可见菜单」的轻量投影，短 TTL（例如 60 秒）。管理员变更菜品上架状态或每日菜单后主动失效对应日期缓存。该方案降低高峰读压力且不引入 Redis 运维依赖；多实例部署时可替换为 Redis 或通过消息广播失效。
...
前端按 `auth`、`menu`、`cart`、`orders`、`settings`、`console` 分 feature 组织；路由守卫同时处理未登录和角色不匹配。所有面向用户的字符串集中在英文文案文件，避免遗漏中文提示。
...
| `daily_menus` | `menu_date` + `dish_id` 唯一、`available_quantity` |
...
4. 服务端根据菜单和选项当前价格计算金额，写入订单、订单项和选项快照；
5. 写入幂等结果后提交事务。

数据库使用唯一约束辅助保护幂等键（`user_id, idempotency_key`）。对「有效订单唯一」使用事务内条件检查，并在 schema 中使用可唯一化的 `active_meal_key`（有效状态为 `date#meal`，取消后为 `NULL`）建立 `(user_id, active_meal_key)` 唯一索引，避免并发请求穿透业务检查。
